...
class Solution {
    ...
    public boolean querySegmentTree(int i , int l , int r , int[] segmentTree , int val){
        if(segmentTree[i] < val){
            return false; // no baskets in this segment
        }
        ...
        segmentTree[i] = Math.max(segmentTree[2 * i + 1] , segmentTree[2 * i + 2]); // because i am changing this above segmentTree[i] = -1
